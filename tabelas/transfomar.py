...
def substituir_require_por_assume(conteudo):
    padrao = re.compile(r"\brequire\s*\((.*?)\)\s*;")

    novo_conteudo = re.sub(padrao, r"vm.assume(\1);", conteudo)
...
novo_conteudo = substituir_require_por_assume(entrada)
salvar_arquivo(novo_conteudo, "arquivo_modificado.sol")
print("Arquivo salvo como 'arquivo_modificado.sol'")
